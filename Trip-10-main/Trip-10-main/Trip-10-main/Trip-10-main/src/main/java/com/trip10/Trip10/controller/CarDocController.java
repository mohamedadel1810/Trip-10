...
public class CarDocController {
    ...
    public ApiResponse<CarDocResponse> verify(@PathVariable int id,@RequestBody CarDocRequest request){

        return carDocService.verifyDoc(id,request);

    }

    @PostMapping("/reject/{id}")
    public ApiResponse<CarDocResponse> reject(@PathVariable int id ,@RequestBody CarDocRequest request){
        return carDocService.rejectDoc(id,request);
    }
}
