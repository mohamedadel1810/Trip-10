...
private CarResponse toCarResponse(Car car){
    ...
    return new CarResponse(car.getCarId(), car.getDocId(), car.getColor(),car.getBrand(), car.getModel(), car.getPlateNumber(),car.getVerificationStatus());
}
    ...
    public ApiResponse<CarResponse> create(CarRequest request) {
        Car cars =new Car();
        cars.setDocId(request.getDocId());
    ...
    @Override
    public ApiResponse<CarResponse> verifyCar(int carId, CarRequest request) {
       Car car= carRepo.findById(carId).orElse(null);

       if (car==null)
           return ApiResponse.notFound("car not found"+carId);

       if (request.getVerificationStatus().equals(VerificationStatus.VERIFIED))
           return ApiResponse.badRequest("car already verified");

       if (request.getVerificationStatus()==VerificationStatus.PENDING)
           request.setVerificationStatus(VerificationStatus.VERIFIED);
       CarResponse saved =toCarResponse(car);
       return ApiResponse.success("Car Verified successfully",saved);
    }
