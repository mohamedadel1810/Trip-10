...
public ApiResponse<DriverDocResponse> getDoc(@PathVariable int id) {return driverDocService.findById(id);}
    ...
    public ApiResponse<DriverDocResponse> verify(@PathVariable int id,int adminId){

        return driverDocService.verifyDoc(id,adminId);

    }
    @PostMapping("/reject/{id}")
    public ApiResponse<DriverDocResponse>reject(@PathVariable int id,@RequestBody DriverDocRequest request,int adminId){
        return driverDocService.rejectDoc(id,request,adminId);
