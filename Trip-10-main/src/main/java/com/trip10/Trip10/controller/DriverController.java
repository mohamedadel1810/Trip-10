...
@PostMapping("/otp/send")
    public ApiResponse<String> sendOTP(@RequestParam String phoneNumber){

        return driverService.sendOtp(phoneNumber);

}
@PostMapping("/verify/{id}")
public ApiResponse<DriverResponse> verifyDriver(@PathVariable int id){
        return driverService.verifyDriver(id);
}

@PostMapping("/phone/verify")
public ApiResponse<DriverResponse>verifyNumber(@RequestParam String phoneNumber,@RequestParam String otpCode){
        return driverService.verifyOtp(phoneNumber,otpCode);
}
