...
public class CarDocController {
    ...
    public ApiResponse<CarDocResponse> verify(@PathVariable int id,@RequestBody CarDocRequest request,int adminId){

        return carDocService.verifyDoc(id,request,adminId);

    }

    @PostMapping("/reject/{id}")
    public ApiResponse<CarDocResponse> reject(@PathVariable int id ,@RequestBody CarDocRequest request,int adminId){
        return carDocService.rejectDoc(id,request,adminId);
    }
}
